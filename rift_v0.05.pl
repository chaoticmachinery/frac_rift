...
#############################################################################################################################################
# Note: Tried converting this to use threading for icat. It made recovery take twice as long as a single thread.
#
#############################################################################################################################################
...
#============================================================================================================================================
sub GetFile {
   my ($line,$savedir,$search_string,$flsopts) = @_;
   ...
   if ($flsline[1] =~ m/$search_string/i) {
		if ($flsline[3] =~ /^d/) {
			...
			print "\nRecoverying File from $host ($ipaddr): $flsline[1]\tCluster/Inode:$flsline[2]\n"; 

            my($filename, $directories, $suffix) = fileparse($flsline[1]);
			my $linkstring = " -> ";
			#$linkstring = "\|l\/l";
			if($directories =~ m/$linkstring/) {
			    my @line = split('->', $directories);
			    $directories = $line[0];
			   	($filename, $directories, $suffix) = fileparse($directories);
			   	$filename .= " (LINK See FLS output)";
            }
			...
			print "ICAT: $icatcmd $flsopts $rootdrv $flsline[2] > \"$savelocation$filename\" \n";
			#print "APFS: $flsopts\n";
			my $error = `$icatcmd $flsopts $rootdrv $flsline[2] > \"$savelocation$filename\"`;
...
switch ($^O) {
    ...
    case "darwin" {
        # OSX boxes
        # Doesn't work as OSX requires the filesystem to be ummounted.
        $win="";
        #$rootdrv=`df / | grep disk | cut -f 1 -d" "`;
        #Assuming Default Macs
        $rootdrv="/dev/rdisk0";
        chomp($rootdrv); 
        print "Root Drive is: $rootdrv\n";
        $pstatapsb = `sudo ./pstat_Darwin -o 409640 /dev/rdisk0 | grep 'APSB' | cut -f 2 -d: | head -n 1`;
        chomp($pstatapsb);
        print "APFS Block: $pstatapsb\n";
        $flsopts="-o 409640 -B $pstatapsb";
        if ($opt_os) {
            print "OS: $opt_os\n";
            $flscmd .= "_".$opt_os;
            $icatcmd .= "_".$opt_os;
        ...
        $win="";
        my $drive  = "/dev/";
        $rootdrv=`grep "/ / " /proc/self/mountinfo`;
        my @words = split /\s+/, $rootdrv;
        foreach $a (@words) {
        if ($a =~ /\/dev\//) {
        $rootdrv = $a;
        }
...
# Main Loop
#=============================================================================================
open (FLSOUT, "$flscmd -m \"\" -r -p $flsopts $rootdrv|") || die "Cannot open $rootdrv for reading. Are you Administator/Root?\n";
open (FLSOUTPUT, "> $flsoutput");
while( $line = <FLSOUT>) {
   print FLSOUTPUT $line;
   chomp($line);
   foreach $search_string (@SrchFileList) {
      GetFile($line,$savedir,$search_string,$flsopts);
   }
}
close(FLSOUTPUT);
close(FLSOUT);
...
=head1 OPTIONS
 ...
 --os {TAG}     Used to ensure the target machine runs the binary related to target machines OS
 --verbose      Verbose output; Used for troubleshooting issues
 --version      Shows version of RIFT
...
=head1 UPDATES
 ...
 2019 Version 0.05 -- Added options for additional OSes
=cut
